...
const SimulationInfoFragment = ({simulations, capacity}) => {
  ...
  ///const adjustedTimeIntegrationFactor = curves.length / (curves[curves.length - 1][0] - curves[0][0]);
  // in this model, each point is a day
  const adjustedTimeIntegrationFactor = 1;
  console.log("adjustedTimeIntegrationFactor:", adjustedTimeIntegrationFactor)
  //
  ...
  const medianBedPeak = indexMedianBedPeak;
  const [lowBedPeak, highBedPeak] = getMinMax(indexBedPeak(indices.highci), indexBedPeak(indices.lowci));
  //
...
export const RunRBinomHospitalSimulation = () => {
  ...
  let curveSDays = moment(simulations.dates.maxEndDate).diff(moment(simulations.dates.minStartDate), "days");
  let curvesS = simulations.curves.map(v => v.S);
  let totalN = jStat.mean(simulations.curves.map(v => v.N[v.N.length - 1]));
  //////let curvesI = simulations.curves.map(v => v.I);
  let populationSize = simulations.curves.map(v => v.I);
  //console.log("")
  //console.log(curvesI)
  //console.log(capacity)
  if (curvesS.length == 0) {
