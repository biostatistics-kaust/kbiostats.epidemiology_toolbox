...
export class TaskRow extends Component {
  ...
  render() {
    let reducedWidth = 0;
    let extraButtons = this.props.extraButtons ? this.props.extraButtons.map(({button, onClick}, idx) => <div key={ `button-container-timeline-${idx}` } className="button-container" onClick={ e => onClick(this.props.item) }>
                                                                                                           { button }
                                                                                                         </div>) : "";
    return (
      <div className="timeLine-side-task-row" style={ { ...Config.values.taskList.task.style, top: this.props.top, height: this.props.itemheight } }>
        { extraButtons }
        <div className="timeLine-side-task-row-label" style={ { width: "100%" } } onClick={ (e) => this.props.onSelectItem(this.props.item) }>
...
export default class TaskList extends Component {
  ...
  renderTaskRow(data) {
    ...
    for (let i = this.props.startRow; i < this.props.endRow + 1; i++) {
      ...
      if (!item) break;
      result.push(
        <TaskRow key={ i } index={ i } item={ item } label={ item.name } top={ i * this.props.itemheight } itemheight={ this.props.itemheight }
          isSelected={ this.props.selectedItem == item } onUpdateTask={ this.props.onUpdateTask } onSelectItem={ this.props.onSelectItem } nonEditable={ this.props.nonEditable } extraButtons={ this.props.extraButtons }
        />
      );
    }
    return result;
